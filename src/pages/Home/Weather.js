        ...
        const getAllWeatherInfo = async () => {
            ...
            if(preferredLocation === "" || preferredLocation.trim().length === 0) {
                ...
                await navigator.geolocation.getCurrentPosition(async (position) => {
                    ...
                    await axios.get(`https://sing-me-the-weather-server-production.up.railway.app/weather/?lat=${lat1}&lon=${lon1}&units=${unit}`)
                        .then(res => {
                ...
                let lon;
                let bad_query = false;
                await axios.get(`https://sing-me-the-weather-server-production.up.railway.app/autocomplete/?search_word=${preferredLocation}`)
                    .then(res => {
                        if (res.data[0]) {
                ...
                if (!bad_query) {
                    await axios.get(`https://sing-me-the-weather-server-production.up.railway.app/place/?place_id=${place_id}`)
                        .then(res => {
                            lat = res.data.lat;
                            lon = res.data.lon;
                    ...
                    await axios.get(`https://sing-me-the-weather-server-production.up.railway.app/weather/?lat=${lat}&lon=${lon}&units=${unit}`)
                        .then(res => {
                            setAllWeatherInfo(res.data);
                            setHoursWeather(res.data.hourly);
                        })
